...
- **문서 기반 AI 채팅**: 등록된 학습 자료를 벡터 검색하여 정확한 답변 생성
- **학습 메타데이터 자동 생성**: 세션 생성 시 학습 목표, 핵심 요약, 추천 질문 자동 생성
- **자동 퀴즈 생성**: 콘텐츠 등록 시 4지선다 및 OX 퀴즈 자동 생성
...
**주요 인덱스**:
- `TB_SESSION`: `(parent_id, course_user_id)` — 자식 세션 중복 방지 조회
...
- 같은 parent + course_user_id 조합이면 기존 자식 세션 반환 (중복 방지)
...
- 현재 테넌트: `dev` (로컬), `user1` (dev와 DB 공유), `user2` (완전 독립)
...
- **채팅 영역**: 메시지 목록 (user/assistant), 자동 스크롤
- **입력 영역**: 텍스트 입력 + 전송 버튼
- **퀴즈**: 4지선다/OX 문제, 오답 시 1회 재시도, 해설 표시
...
8. **콘텐츠 등록 흐름**: 텍스트 추출 → DB 저장 → 청크 분할 → 임베딩 → Vectorize 저장 → (백그라운드) 퀴즈 생성
...
cd malgn-chatbot-api && wrangler deploy --env user2
