...
| **AI 모델 (채팅/학습/퀴즈)** | `@cf/google/gemma-3-12b-it` |
...
malgn-chatbot-user1/    ← user1 테넌트 프론트엔드 (메인 도메인 의존: chatbot-embed.js를 malgn-chatbot.pages.dev에서 로드)
malgn-chatbot-cloud/    ← cloud 테넌트 프론트엔드 (자체 호스팅: js/css 번들 포함)
malgn-chatbot-secbiz/   ← secbiz 테넌트 프론트엔드 (자체 호스팅: js/css 번들 포함)
```
...
│   │   ├── embeddingService.js # 텍스트→벡터 변환 (1024차원, bge-m3)
...
- 각 테넌트별 독립: Aurora MySQL DB(Hyperdrive 바인딩), KV 네임스페이스, R2 버킷, Vectorize 인덱스
- 배포: `wrangler deploy --env <tenant_id>`
- 현재 테넌트: `dev` (로컬, MySQL/Hyperdrive), `user1` (dev와 MySQL 공유), `cloud` (전용 MySQL/Hyperdrive 독립), `secbiz` (전용 MySQL/Hyperdrive 독립)
...
cd malgn-chatbot-api && wrangler deploy --env secbiz
# 또는 일괄 배포: ./scripts/deploy.sh --all

# 스키마 적용 (모든 테넌트 Aurora MySQL — Hyperdrive 통한 직접 SQL)
mysql -h <HOST> -u <USER> -p <DATABASE> < schema.mysql.sql
# 마이그레이션 파일은 순서대로 직접 적용
