...
`});var u,E=p(()=>{C();u={chatbot:null,fab:null,root:null,isInline:!1,inject(c){let t=c.width||380,e=c.height||650;this.isInline=c.mode==="inline";let o=document.createElement("div");o.id="malgn-tutor-host",this.isInline&&(o.style.cssText="display:block;width:100%;height:100%;");let a=o.attachShadow({mode:"open"});this.root=a;let n=document.createElement("style");n.textContent=I,a.appendChild(n);let r=document.createElement("link");if(r.rel="stylesheet",r.href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.1/font/bootstrap-icons.css",a.appendChild(r),!this.isInline){let s=document.createElement("button");s.id="malgn-fab",s.className="chat-fab",s.title="\uCC44\uD305 \uC5F4\uAE30",s.innerHTML=`
        <i class="bi bi-chat-dots-fill chat-fab-icon"></i>
        <i class="bi bi-x-lg chat-fab-close"></i>
      `,a.appendChild(s),this.fab=s}let i=document.createElement("div");if(i.id="malgn-chatbot",this.isInline?i.className="chatbot chatbot--inline":(i.className="chatbot",i.hidden=!0,i.style.width=t+"px",i.style.height=e+"px"),i.innerHTML=`
      <!-- Header -->
    ...
    `;if(n)e.options.forEach((l,b)=>{let g=b+1,f=this.answers[e.id]===String(g);r+=`
    ...
    `,t.innerHTML=r,t.querySelectorAll(".chatbot-quiz-option").forEach(l=>{l.addEventListener("click",()=>{this.answers[l.dataset.quizId]=l.dataset.answer,this.renderCurrentQuiz()})});let i=this.root.querySelector("#malgn-prev-quiz"),s=this.root.querySelector("#malgn-next-quiz"),d=this.root.querySelector("#malgn-check-answer");i&&i.addEventListener("click",()=>this.prev()),s&&s.addEventListener("click",()=>this.next()),d&&d.addEventListener("click",()=>this.checkAnswer())}prev(){this.currentIndex>0&&(this.currentIndex--,this.renderCurrentQuiz())}next(){this.currentIndex<this.quizzes.length-1&&(this.currentIndex++,this.renderCurrentQuiz())}checkAnswer(){let t=this.quizzes[this.currentIndex],e=this.answers[t.id],o=this.root.querySelector("#malgn-quiz-result");if(!o)return;if(!e){o.className="chatbot-quiz-result",o.innerHTML='<span class="text-warning">\uB2F5\uC744 \uC120\uD0DD\uD574 \uC8FC\uC138\uC694.</span>',o.style.display="block";return}this.attempts[t.id]=(this.attempts[t.id]||0)+1;let a=this.attempts[t.id],n=e===t.answer,r=n?"chatbot-result-correct":"chatbot-result-wrong",i=n?"bi-check-circle-fill":"bi-x-circle-fill",s=n?"\uC815\uB2F5\uC785\uB2C8\uB2E4.":"\uC624\uB2F5\uC785\uB2C8\uB2E4.";o.className=`chatbot-quiz-result ${n?"correct":"wrong"}`;let d=`
      <div class="${r}">
        <i class="bi ${i} chatbot-result-icon"></i>${s}
      </div>
    `;if(!n&&a===1&&(d+='<div class="chatbot-result-explanation">\uB2E4\uC2DC \uD55C \uBC88 \uB3C4\uC804\uD574 \uBCF4\uC138\uC694!</div>'),(n||a>=2)&&t.explanation&&(d+=`<div class="chatbot-result-explanation"><strong>\uD574\uC124:</strong> ${h(t.explanation)}</div>`),o.innerHTML=d,o.style.display="block",n||a>=2){this.checked[t.id]=!0;let l=this.root.querySelector("#malgn-next-quiz"),b=this.root.querySelector("#malgn-check-answer");l&&this.currentIndex<this.quizzes.length-1&&(l.style.display=""),b&&(b.style.display="none")}}}});var A=B(()=>{S();E();L();T();M();if(window.__malgnTutorLoaded)console.warn("[MalgnTutor] Already loaded, skipping.");else{let c=function(){let t=window.MalgnTutor;if(!t||!t.apiUrl){console.error("[MalgnTutor] window.MalgnTutor.apiUrl is required.");return}let e=t.mode==="inline";if(!document.querySelector('link[href*="bootstrap-icons"]')){let s=document.createElement("link");s.rel="stylesheet",s.href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.1/font/bootstrap-icons.css",document.head.appendChild(s)}let o=new y(t.apiUrl,t.apiKey);u.inject({mode:t.mode||"layer",container:t.container||null,title:t.title||"",videoIframeId:t.videoIframeId||"",width:t.width||380,height:t.height||650});let a=u.root,n=new q(a);n.init();let r=new k(o,a),i=new z(o,{contentIds:t.contentIds||[],courseId:t.courseId||0,courseUserId:t.courseUserId||0,lessonId:t.lessonId||0,userId:t.userId||0,settings:t.settings||{},parentSessionId:t.parentSessionId||0},a);if(i.init(),i.onSessionCreating=()=>{u.setFabLoading(!0)},i.onSessionCreated=s=>{u.setFabLoading(!1),s.learning&&n.renderLearningData(s.learning),s.session&&s.session.id&&r.loadQuizzes(s.session.id),s.messages&&s.messages.length>0&&(i.clearMessages(),s.messages.forEach(d=>{d.role==="user"?i.addUserMessage(d.content):i.addAssistantMessage(d.content)}))},i.onSessionLoaded=s=>{s.learning&&n.renderLearningData(s.learning),s.id&&r.loadQuizzes(s.id)},n.onQuestionClick=s=>{i.sendMessage(s)},t.sessionId&&i.loadSession(t.sessionId),!e){a.querySelector("#malgn-fab").addEventListener("click",()=>u.toggle());let s=a.querySelector("#malgn-close");s&&s.addEventListener("click",d=>{d.preventDefault(),d.stopPropagation(),u.close()})}console.log(`[MalgnTutor] Initialized (${e?"inline":"layer"} mode, Shadow DOM).`)};window.__malgnTutorLoaded=!0,document.readyState==="loading"?document.addEventListener("DOMContentLoaded",c):c()}});A();})();
